...
def kg_rag_parallel(data, doc2kg, top_k=5, workers=4,persist_dir=None,reranker='../model/bge-reranker-large'):
    ...
    kg_post_processor1 = KGRetrievePostProcessor(dataset="hotpotqa",ents=ents,doc2kg=doc2kg,chunks_index=chunks_index)
    bge_reranker = FlagReranker(model_name_or_path=reranker, cache_dir=r"Q:/hf_models")
    kg_post_processor2 = GraphFilterPostProcessor(dataset="hotpotqa",topk=top_k,ents=ents,doc2kg=doc2kg,chunks_index=chunks_index,reranker=bge_reranker)

    engine = RetrieverQueryEngine(retriever=retriever,response_synthesizer=response_synthesizer,node_postprocessors=[kg_post_processor1,kg_post_processor2,NaivePostprocessor(dataset="hotpotqa")])
...
def main(args):
    ...
    # )
    # Settings.llm = OpenAI(api_base="http://127.0.0.1:19002/v1", model=args.model_name, request_timeout=200, api_key="xd")
    Settings.llm = OpenAI(api_base="https://api.avalai.ir/v1", api_key="", model="gemini-2.5-flash", request_timeout=200)
    print(Settings.llm)
    embed_model_name = args.embed_model_name
    print('Init Ollama embedding')
    # Settings.embed_model = OpenAIEmbedding(api_base="http://127.0.0.1:19001/v1", model_name=args.embed_model_name, api_key="xd")
    Settings.embed_model = HuggingFaceEmbedding(
        model_name="Qwen/Qwen3-Embedding-0.6B",  # You can use any HF embedding model
        cache_folder="Q:/hf_models",
        model_kwargs={
    ...
    result_path = args.result_path+"_"+str(args.top_k)+".json"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path',type=str,default='../data/pquad/PQuad_json.json',help='Path to the data file')
    parser.add_argument('--result_path',type=str,default='../output/pquad/pquad_train_distractor_v1_full_gemini',help='Path to the result file')
    parser.add_argument('--kg_dir',type=str,default='../data/pquad/kgs/extract_subkgs')
    parser.add_argument('--persist_dir',type=str,default='../data/qwen/pquad',help='Directory to store the index')
    ...
    parser.add_argument('--top_k',type=int,default=5,help='Top k similar documents')
    parser.add_argument('--num_workers',type=int,default=2,help='Number of workers for parallel processing')
    parser.add_argument('--reranker',type=str,default='BAAI/bge-reranker-base',help='Path of the reranker model')
