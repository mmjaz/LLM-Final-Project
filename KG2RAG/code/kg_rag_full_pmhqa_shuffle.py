...
def kg_rag_parallel(data, doc2kg, top_k=5, workers=4,persist_dir=None,reranker='../model/bge-reranker-large'):
    ...
    for sample in tqdm(data[:min(len(data),test_size)]):
        sample_id = sample['id']
        if sample_id not in [54, 51, 52, 55, 56, 57, 1157, 58, 59, 60, 62]:
            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path',type=str,default=r'G:\jupyter\pycharm\LLM-Final-Project\data\pmhqa\all_persian_mhqa_shuffle.json',help='Path to the data file')
    parser.add_argument('--result_path',type=str,default='../output/pmhqa_shuffle/pmhqa_train_distractor_v1_full_gemini',help='Path to the result file')
    parser.add_argument('--kg_dir',type=str,default='../data/persianmhqa_shuffle/kgs/extract_subkgs')
    parser.add_argument('--persist_dir',type=str,default='../data/qwen/pmhqa_shuffle',help='Directory to store the index')
