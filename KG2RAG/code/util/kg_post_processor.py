    ...
    def _postprocess_nodes(
        ...
        for node in nodes:
            if self.dataset == 'hotpotqa':
                ...
                idx_str, ent, seq_str = node.node.id_.split('##')
                idx_seq_str = f'{idx_str}##{seq_str}'
            if (ent not in self.doc2kg) or (idx_seq_str not in self.doc2kg[ent]) or (
                    len(self.doc2kg[ent][idx_seq_str]) == 0):
                continue
            for triplet in self.doc2kg[ent][idx_seq_str]:
                h, r, t = triplet
                h = h.strip()
        ...
        for i in range(len(sorted_wccs)):
            ...
            if len(wcc) > 1:
                ...
                cand_ctxs_list.append(list(set(cand_ctx_list)))
            else:
        ...
        for cand_ids_list in cand_ids_lists:
            ...
            for cand_id in cand_ids_list:
                ...
                if (self.use_tpt) and (cand_ent in self.doc2kg) and (idx_seq_str in self.doc2kg[cand_ent]) and (
                        len(self.doc2kg[cand_ent][idx_seq_str]) > 0):
                    tpt_str += ', '.join([f'{h} دارد/هست {r} {t}' for h, r, t in self.doc2kg[cand_ent][idx_seq_str][
                        :min(len(self.doc2kg[cand_ent][idx_seq_str]), 3)]])
                    if len(tpt_str) > 0:
